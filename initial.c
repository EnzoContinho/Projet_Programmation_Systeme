...
int main(int argc, char *argv[], char **envp){
  ...
  if(nb_archivistes < 2 || nb_archivistes > 100 || nb_themes < 2 || nb_themes > 100){
    ...
    * Avec une nouvelle clé à chaque fois */
    ...
    struct sigaction gestio;
    gestio.sa_handler = &handler;
    gestio.sa_flags = 0;
    sigemptyset(&gestio.sa_mask);

    sigaction(SIGINT,&gestio,NULL);
    sigaction(SIGQUIT,&gestio,NULL);
    sigaction(SIGUSR1,&gestio,NULL);
    sigaction(SIGUSR2,&gestio,NULL);
    sigaction(SIGTERM,&gestio,NULL);
